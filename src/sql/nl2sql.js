...
export async function generateSqlFromQuestion(question, context = null) {
  ...
  const shouldUseContext =
    context &&
    /con lai|còn lại|the con|thế còn|cac khoa con lai|các khoa còn lại/i.test(
...
export async function answerWithSqlPlan(question, plan) {
  if (!dbReady || !pool)
    return {
      ok: false,
      reply: "MySQL chưa kết nối nên chưa kiểm tra được dữ liệu.",
    };
  if (!plan || !plan.sql)
    return { ok: false, reply: plan?.reason || "Không tạo được SQL." };
  ...
  if (!validation.ok)
    return { ok: false, reply: `SQL bị chặn: ${validation.reason}` };
  ...
  try {
    ...
    return {
      ok: false,
      reply: "Không truy vấn được dữ liệu. Vui lòng thử câu hỏi khác.",
    };
  }
}
...
export async function answerWithSql(question, context = null) {
  ...
  try {
    ...
    if (!plan.sql)
      return { ok: false, reply: plan.reason || "Model không tạo được SQL." };
