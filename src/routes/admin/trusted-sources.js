...
router.get("/api/admin/trusted-sources", requireAdmin, requireDb, async (req, res) => {
...
router.post("/api/admin/trusted-sources", requireAdmin, requireDb, async (req, res) => {
...
router.put("/api/admin/trusted-sources/:id", requireAdmin, requireDb, async (req, res) => {
...
router.delete("/api/admin/trusted-sources/:id", requireAdmin, requireDb, async (req, res) => {
  const id = Number(req.params.id);
  if (!id) return res.status(400).json({ error: "Thiếu id." });
