...
router.post("/api/admin/schema", requireAdmin, requireDb, async (req, res) => {
  ...
  try {
    columnsStr = JSON.stringify(
      typeof req.body.columns_json === "string"
        ? JSON.parse(req.body.columns_json)
        : req.body.columns_json,
    );
    examplesStr = JSON.stringify(
      typeof req.body.examples_json === "string"
        ? JSON.parse(req.body.examples_json || "[]")
        : req.body.examples_json || [],
    );
  } catch {
