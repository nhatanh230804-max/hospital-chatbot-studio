...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!name || !questionPattern || !sqlTemplate) {
      return res
        .status(400)
        .json({ error: "Thiếu name/question_pattern/sql_template." });
    }
    ...
    if (!keywords) {
      ...
      if (!keywords) {
        return res
          .status(400)
          .json({
            error: "Không tạo được keywords tự động. Vui lòng nhập thủ công.",
          });
      }
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!validation.ok) {
      return res
        .status(400)
        .json({ error: `SQL không hợp lệ: ${validation.reason}` });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await pool.execute(
      `SELECT * FROM sql_templates WHERE id = ?`,
      [id],
    );
    if (!rows[0])
    ...
    const tpl = rows[0];
    const resolvedSql = resolvePlaceholders(
      tpl.sql_template,
      testQuestion || tpl.question_pattern,
    );

    const validation = await validateAndPrepareSql(
      resolvedSql,
      tpl.connection_id,
      tpl.connection_database,
    );
    if (!validation.ok) {
      return res.json({
        ok: false,
    ...
    try {
      const resultRows = await runSqlOnScope(
        validation.sql,
        tpl.connection_id,
        tpl.connection_database,
      );
      res.json({
        ...
        sql: validation.sql,
        rows: resultRows,
        reply: await summarizeSqlResult(
          testQuestion || tpl.question_pattern,
          validation.sql,
          resultRows,
        ),
        scope: tpl.connection_id
          ? `connection #${tpl.connection_id} · ${tpl.connection_database || "default"}`
          : "DB chính",
      });
    } catch (error) {
