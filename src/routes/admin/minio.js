...
router.post("/api/admin/minio/:connectionId/sync", requireAdmin, requireDb, async (req, res) => {
...
// List indexed files
router.get("/api/admin/minio-files", requireAdmin, requireDb, async (req, res) => {
...
router.put("/api/admin/minio-files/:id", requireAdmin, requireDb, async (req, res) => {
...
router.delete("/api/admin/minio-files/:id", requireAdmin, requireDb, async (req, res) => {
  const id = Number(req.params.id);
  if (!id) return res.status(400).json({ error: "Thiếu id." });
...
router.post("/api/admin/minio-files/:id/url", requireAdmin, requireDb, async (req, res) => {
  const id = Number(req.params.id);
  if (!id) return res.status(400).json({ error: "Thiếu id." });
