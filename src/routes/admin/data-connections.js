...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(
      rows.map((r) => ({
        ...r,
        config_json: redactConfigForRead(
          r.type,
          safeJsonParse(r.config_json, {}),
        ),
      })),
    );
  }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!name || !type)
      return res.status(400).json({ error: "Thiếu name hoặc type." });
    try {
      getAdapter(type);
    } catch (err) {
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [oldRows] = await pool.execute(
      `SELECT type, config_json FROM data_connections WHERE id = ?`,
      [id],
    );
    if (!oldRows.length)
