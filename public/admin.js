...
async function loadConnectionsForDropdown() {
  // Cache 30s
  if (_connectionsCache && Date.now() - _connectionsCacheAt < 30000)
  ...
  try {
    ...
    _connectionsCache = list.filter(
      (c) => c.is_active && ["mysql", "postgres"].includes(c.type),
    );
...
async function populateConnectionDropdown(
  ...
  select.innerHTML =
    ...
    connections
      .map((c) => {
        const selected =
          String(c.id) === String(currentConnectionId) ? "selected" : "";
...
async function suggestKeywords(
  ...
  try {
    ...
    toast(
      `Đã sinh ${data.keywords.length} keywords (${data.method || "heuristic"})`,
      "success",
    );
  } catch (err) {
    kwEl.value = originalValue;
...
const tabs = {
  dashboard: {
    ...
    subtitle: "Tổng quan trạng thái hệ thống chatbot bệnh viện",
    loader: loadDashboard,
  },
  faq: {
    title: "FAQ đã duyệt",
    subtitle:
      "Quản lý câu hỏi - câu trả lời tham khảo (upload file hoặc nhập tay)",
    loader: loadFaq,
  },
  schema: {
    title: "Schema (Dạy bảng)",
    subtitle: "Mô tả bảng/cột để chatbot hiểu khi tạo SQL",
    loader: loadSchema,
  },
  templates: {
    title: "Dạy SQL · Templates",
    subtitle: "Hàm SQL mẫu — chatbot ưu tiên match template trước khi gọi AI",
    loader: loadTemplates,
  },
  sources: {
    title: "Nguồn tra cứu",
    subtitle:
      "Whitelist URL mà chatbot được phép tham khảo (Research Mode + Fallback)",
    loader: loadSources,
  },
  connections: {
    title: "Kết nối DB / Storage",
    subtitle:
      "Quản lý kết nối tới MySQL, PostgreSQL, MinIO và các nguồn dữ liệu ngoài",
    loader: loadConnections,
  },
  miniofiles: {
    title: "File trên MinIO",
    subtitle:
      "Danh sách object đã index từ MinIO bucket — gán keywords để chatbot tìm được",
    loader: loadMinioFiles,
  },
  feedback: {
    title: "Feedback",
    subtitle: "Góp ý từ người dùng — duyệt để bổ sung vào FAQ",
    loader: loadFeedback,
  },
  playground: {
    title: "SQL Playground",
    subtitle: "Thử câu hỏi → AI sinh SQL → chạy thật trên DB",
    loader: loadPlayground,
  },
  cache: {
    title: "Research Cache",
    subtitle: "Cache câu trả lời từ Research Mode (TTL 7 ngày)",
    loader: loadCache,
  },
  logs: {
    title: "Logs",
    subtitle: "Nhật ký chat gần đây — 200 dòng mới nhất",
    loader: loadLogs,
  },
};
...
async function loadDashboard() {
  try {
    ...
    setTile(
      "tileDb",
      data.dbReady ? "Online" : "Offline",
      data.dbReady ? "Kết nối tốt" : "Mất kết nối",
      data.dbReady ? "ok" : "bad",
    );
    setTile(
      "tileLlm",
      data.anythingLLMConfigured ? "Online" : "Chưa cấu hình",
      data.anythingLLMConfigured ? "Đã set API key" : "Cần cấu hình .env",
      data.anythingLLMConfigured ? "ok" : "warn",
    );
    setTile(
      "tileFeedback",
      data.feedbackPending,
      "Đang chờ duyệt",
      data.feedbackPending > 0 ? "warn" : "ok",
    );
    ...
    setTile(
      ...
      data.templateTotal,
      "Đang active",
      data.templateTotal > 0 ? "ok" : "warn",
    );
    setTile(
      "tileSources",
      data.sourceTotal,
      "Đang active",
      data.sourceTotal > 0 ? "ok" : "bad",
    );
    setTile("tileSchema", data.schemaTotal, "Bảng được dạy");
    setTile("tileCache", data.cacheTotal, "Còn hiệu lực");
    setTile(
      "tileConnections",
      data.connectionTotal ?? 0,
      "DB/Storage active",
      (data.connectionTotal ?? 0) > 0 ? "ok" : "warn",
    );
...
async function loadFaq() {
  ...
  $("faqUploadFile").addEventListener("change", (e) => {
    ...
    if (!f) {
      $("faqFileSelected").innerHTML = "";
      return;
    }
    $("faqFileSelected").innerHTML =
      `<div class="file-selected"><span>📎 ${escapeHtml(f.name)} (${Math.round(f.size / 1024)} KB)</span><button class="btn ghost sm" type="button" onclick="document.getElementById('faqUploadFile').value=''; document.getElementById('faqFileSelected').innerHTML='';">Bỏ chọn</button></div>`;
  });
  ...
  try {
    ...
    if (!list.length) {
      $("faqListContainer").innerHTML =
        '<div class="empty"><div class="emoji">📋</div>Chưa có FAQ nào.</div>';
      return;
    }
    $("faqListContainer").innerHTML = `
...
function faqRow(f) {
  const sourceLabel = f.source_file_name
    ? `<span class="badge teal">📄 file</span><div class="help" style="margin-top:3px">${escapeHtml(f.source_file_name)}</div>`
    : '<span class="badge gray">tay</span>';
...
async function faqDelete(id) {
  const ok = await confirmDialog(
    "Xoá FAQ này? File upload (nếu có) cũng sẽ bị xoá.",
  );
  if (!ok) return;
  try {
  ...
  try {
    ...
    if (!list.length) {
      $("schemaListContainer").innerHTML =
        '<div class="empty"><div class="emoji">🗂️</div>Chưa có bảng nào được dạy.</div>';
      return;
    }
    $("schemaListContainer").innerHTML = `
      <table class="data-grid">
...
async function schemaDelete(id) {
  const ok = await confirmDialog(
    'Xoá schema này khỏi hệ thống? Hành động này không thể hoàn tác. Nếu cần khôi phục, hãy bấm "📚 Import bảng" trên connection để re-import.',
  );
  if (!ok) return;
  try {
  ...
  try {
    ...
    if (!list.length) {
      $("templateListContainer").innerHTML =
        '<div class="empty"><div class="emoji">⚙️</div>Chưa có template nào.</div>';
      return;
    }
    $("templateListContainer").innerHTML = `
      <table class="data-grid">
...
function templateRow(t) {
  ...
  const cat = t.category
    ? `<span class="badge teal">${escapeHtml(t.category)}</span>`
    : "";
  const lastUsed = t.last_used_at
    ? `<div class="help">${new Date(t.last_used_at).toLocaleString("vi-VN")}</div>`
    : "";
...
async function sourceCreate() {
  try {
    ...
    ["src_name", "src_url", "src_desc"].forEach((id) => {
      $(id).value = "";
    });
  ...
  try {
    ...
    if (!list.length) {
      $("sourceListContainer").innerHTML =
        '<div class="empty"><div class="emoji">🔗</div>Chưa có nguồn nào. Hãy thêm ít nhất 1 nguồn để Research Mode hoạt động.</div>';
      return;
    }
    $("sourceListContainer").innerHTML = `
      <table class="data-grid">
  ...
  try {
    ...
    if (!list.length) {
      $("feedbackListContainer").innerHTML =
        '<div class="empty"><div class="emoji">✓</div>Không có feedback nào đang chờ duyệt.</div>';
      return;
    }
  ...
  try {
    ...
    if (!all.length) {
      $("feedbackHistoryContainer").innerHTML =
        '<div class="empty">Chưa có feedback đã xử lý.</div>';
      return;
    }
    $("feedbackHistoryContainer").innerHTML = `
      <table class="data-grid">
...
async function playgroundRun() {
  ...
  try {
    ...
    $("play_result").innerHTML =
      `<span class="badge red">Lỗi</span><pre>${escapeHtml(err.message)}</pre>`;
  }
}
  ...
  try {
    ...
    if (!list.length) {
      $("cacheListContainer").innerHTML =
        '<div class="empty"><div class="emoji">💾</div>Cache trống.</div>';
      return;
    }
    $("cacheListContainer").innerHTML = `
      <table class="data-grid">
  ...
  try {
    ...
    if (!list.length) {
      $("logsListContainer").innerHTML =
        '<div class="empty"><div class="emoji">📜</div>Chưa có log nào.</div>';
      return;
    }
    $("logsListContainer").innerHTML = `
      <table class="data-grid">
...
function routeColor(route) {
  ...
  if (route === "sql-template" || route === "faq" || route === "document")
    return "green";
...
async function collectConnConfig(type, idPrefix = "conn_cfg_") {
  ...
  for (const f of adapter.configSchema) {
    ...
    if (f.type === "boolean") cfg[f.key] = el.checked;
      ...
      cfg[f.key] = Number(el.value) || (f.default ?? 0);
  ...
  try {
    ...
    if (!list.length) {
      $("connectionListContainer").innerHTML =
        '<div class="empty"><div class="emoji">🔌</div>Chưa có kết nối nào.</div>';
      return;
    }
    $("connectionListContainer").innerHTML = `
      <table class="data-grid">
...
async function connectionTest(id) {
  ...
  try {
    const res = await api(`/api/admin/data-connections/${id}/test`, {
      method: "POST",
    });
...
async function connectionResources(id) {
  openModal(
    `<h2>Resources</h2><div id="m_conn_resources"><span class="muted">Đang load...</span></div><div class="modal-actions"><button class="btn ghost" onclick="closeModal(false)">Đóng</button></div>`,
  );
  try {
    ...
    $("m_conn_resources").innerHTML =
      `<div class="badge red">${escapeHtml(err.message)}</div>`;
  }
}
...
async function connectionSyncMinio(id) {
  ...
  try {
    ...
    if (res.missingFiles && res.missingFiles.length > 0) {
      ...
      if (
        document
          .getElementById("tab-minio-files")
...
function missingFilesSelectAll(checked) {
  document
    .querySelectorAll("#missingFilesList input[type=checkbox]")
    .forEach((c) => {
      c.checked = checked;
    });
}
...
async function missingFilesSubmit(connectionId) {
  const checked = Array.from(
    document.querySelectorAll("#missingFilesList input[type=checkbox]:checked"),
  );
  ...
  try {
    ...
    if (
      document.getElementById("tab-minio-files")?.classList?.contains("active")
    ) {
      loadMinioFiles();
    }
  } catch (err) {
...
async function connectionImportSchema(id, defaultDatabase = "") {
  ...
  try {
    const data = await api("/api/admin/list-tables", {
      ...
      body: JSON.stringify({
        connection_id: id,
        connection_database: defaultDatabase || null,
      }),
    });
    if (!data.ok)
      return toast("Quét fail: " + (data.error || "unknown"), "error");
    if (!data.tables.length)
      return toast("Không tìm thấy bảng nào trong connection này.", "error");
...
async function connectionDelete(id) {
  const ok = await confirmDialog(
    "Xoá kết nối này? Các file MinIO đã index sẽ bị xoá theo.",
  );
  if (!ok) return;
  try {
...
async function minioFileLink(id) {
  try {
    const res = await api(`/api/admin/minio-files/${id}/url`, {
      method: "POST",
    });
...
async function minioFileDelete(id) {
  const ok = await confirmDialog(
    "Xoá metadata file này khỏi index? (File thật trên MinIO không bị xoá)",
  );
  if (!ok) return;
  try {
