...
const VN_STOPWORDS = new Set([
  ...
  "cua", "tren", "duoi", "trong", "ngoai", "voi", "cho", "tu", "den",
  "khi", "thi", "neu", "hoac", "va", "hay", "nhung", "boi", "vi", "ma",
  // Common verbs
  "co", "khong", "duoc", "phai", "moi", "lam", "den",
  // Pronouns
  "cac", "nhung", "nay", "kia", "ay", "ban", "minh", "toi", "anh", "em",
  // Connectors
  "la", "rang", "luc", "thoi", "lan", "phan", "muc", "het",
  // Generic time
  "ngay", "thang", "nam",
  // Question words (giữ trong câu nhưng không thành keyword)
  "ai", "gi", "nao", "dau", "bao", "may",
  // Misc
  "mot", "hai", "ba", "bon", "nam", "sau", "bay", "tam", "chin", "muoi",
  "the", "do", "se", "dang", "da", "roi", "den", "tat", "ca"
]);
...
function extractKeywordsHeuristic(text, opts = {}) {
 ...
 const maxKeywords = opts.maxKeywords || (opts.source === "filename" ? 10 : 8);
  ...
  const tokens = source === "filename"
    ? [...leafTokens, ...parentTokens, ...tokenize(opts.additionalContext || "")]
  ...
  for (const kw of sorted) {
    ...
    const containsExisting = final.some(
      (existing) => kw.length > existing.length && kw.includes(existing) && existing.split(" ").length >= 2
    );
...
function generateSchemaFromDescribe(tableName, describeRows) {
  ...
  const domain = normalize(humanizeColumnName(tableName)).split(" ")[0] || "data";
