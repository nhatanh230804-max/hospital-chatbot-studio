...
export const mysqlAdapter = {
  ...
  configSchema: [
    ...
    { key: "port", label: "Port", default: 3306, type: "number", required: true },
  ...
  async testConnection(config) {
    ...
    try {
      ...
      await conn.end();
      return { ok: true, message: `Kết nối OK`, details: { version: rows[0].version } };
    } catch (err) {
...
async listResources(config) {
    ...
    try {
      ...
      for (const t of tables.slice(0, 50)) {
        try {
          const [countRows] = await conn.query(`SELECT COUNT(*) AS cnt FROM \`${t.TABLE_NAME}\``);
          results.push({
            name: t.TABLE_NAME,
            type: "table",
...
export const postgresAdapter = {
  ...
  configSchema: [
    { key: "host", label: "Host", default: "127.0.0.1", required: true },
    { key: "port", label: "Port", default: 5432, type: "number", required: true },
    { key: "user", label: "User", required: true },
    { key: "password", label: "Password", type: "password", required: true },
    { key: "database", label: "Database", required: true }
  ],
...
export const minioAdapter = {
  ...
  configSchema: [
    { key: "endpoint", label: "Endpoint host", default: "127.0.0.1", required: true, help: "Chỉ host, không có http://" },
    { key: "port", label: "Port", default: 9000, type: "number", required: true },
  ...
  async testConnection(config) {
    try {
      ...
      return { ok: true, message: `Kết nối OK · bucket "${config.bucket}" hợp lệ` };
    } catch (err) {
      return { ok: false, message: err.message };
    }
  },
  ...
  async presignedUrl(config, objectKey, expirySeconds = 3600) {
    const client = this._makeClient(config);
    return await client.presignedGetObject(config.bucket, objectKey, expirySeconds);
  },
